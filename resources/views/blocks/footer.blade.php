<footer>
    <div class="container">
        <div class="footer-content">

            <img class="footer-content__logo" src="{{ asset('/assets/icons/logo-white.svg') }}" alt="fluid-line">

            <div class="footer-content__links footer-links">
                <a href="https://fluid-line.ru/about" class="footer-links__link">
                    О нас
                </a>
                <a href="https://fluid-line.ru/agreement" class="footer-links__link">
                    Политика конфиденциальности
                </a>
            </div>

            <div class="footer-content__contacts footer-contacts">
                <a href="tel:74957411291" class="footer-contacts__phone">
                    +7 (495) 741-12-91
                </a>
                <a href="mailto:kurs@fluid-line.ru" class="footer-contacts__email">
                    kurs@fluid-line.ru
                </a>
            </div>

            <div class="footer-content__socials footer-socials">
                <a class="footer-socials__item footer-socials__web">
                    <img class="footer-socials__img" src="{{ asset('/assets/icons/socials/web.svg') }}" alt="web">
                </a>
                <a class="footer-socials__item footer-socials__youtube" href="https://www.youtube.com/channel/UCsD72HATFzpKD0kJJPjF57A">
                    <img class="footer-socials__img" src="{{ asset('/assets/icons/socials/youtube.svg') }}" alt="youtube">
                </a>
                <a class="footer-socials__item footer-socials__vk" href="https://vk.com/fluidline">
                    <img class="footer-socials__img" src="{{ asset('/assets/icons/socials/vk.svg') }}" alt="vk">
                </a>
                <a class="footer-socials__item footer-socials__dzen" href="https://dzen.ru/profile/editor/id/5f7ad0ff21a4da6245a6c83f">
                    <img class="footer-socials__img" src="{{ asset('/assets/icons/socials/dzen.svg') }}" alt="dzen">
                </a>
                <a class="footer-socials__item footer-socials__telegram" href="https://t.me/fluidline_ru/">
                    <img class="footer-socials__img" src="{{ asset('/assets/icons/socials/telegram.svg') }}" alt="telegram">
                </a>
                <a class="footer-socials__item footer-socials__ok" href="https://ok.ru/profile/581544468815">
                    <img class="footer-socials__img" src="{{ asset('/assets/icons/socials/ok.svg') }}" alt="ok">
                </a>
            </div>   

            <div class="footer-content__year">
                © {{ now()->year }}
            </div>

        </div>
    </div>
</footer>
